...
const CategoryListItemContainer = ({
  ...
  const { onClick } = useRouteNavigate(`/${moldevId}/${item.postInfo.id}`);
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col items-start justify-between bg-white px-16 py-13">
        <div className="shrink-0 w-full font-bold text-16">
          {item.postInfo.title}
        </div>
        <div className={`shrink-0 w-full grow text-14 line-clamp-1`}>
          {item.postInfo.content}
        </div>
        <div className="shrink-0 w-full flex items-center justify-end text-12 text-gray-800">{`조회수 ${item.postInfo.viewCount}회 | ${item.postInfo.lastModifiedDate}`}</div>
      </div>
    </div>
  );
};
